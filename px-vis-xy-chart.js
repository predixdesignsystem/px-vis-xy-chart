...
import './css/px-vis-xy-chart-styles.js';
  ...
  _template: html`
    ...
    <div id="wrapper" class\$="{{_chartWrapperClass}}">
      ...
      <div class\$="{{_registerWrapperClass}} safari-flex-fix">
        <px-vis-register id="register" class\$="{{_getHideClass(hideRegister)}}" height="[[_registerHeight]]" dynamic-menu-config="[[dynamicMenuConfig]]" tooltip-data="[[_registerTooltipData]]" chart-data="[[_filteredData]]" complete-series-config="[[completeSeriesConfig]]" muted-series="{{mutedSeries}}" type="{{_registerType}}" x-axis-type="[[xAxisType]]" y-axis-type="[[yAxisType]]" current-page="[[_registerCurrentPage]]" total-pages="[[_registerTotalPages]]" display-page-arrows="[[_registerDisplayPageArrows]]">
        </px-vis-register>
        <div id="drawingWrapper" class="flex--col inline--flex">
          <px-vis-toolbar id="toolbar" current-sub-config="[[toolbarSubConfig]]" action-config="{{actionConfig}}" within-chart="" chart-margin="[[_internalMargin]]" show-tooltip="{{_internalShowTooltip}}" config="{&quot;zoom&quot;: true, &quot;pan&quot;: true, &quot;tooltip&quot;: true}">
          </px-vis-toolbar>
          <!-- Core -->
          <px-vis-tooltip id="tooltip" tooltip-data="[[tooltipData]]" chart-data="[[_filteredData]]" muted-series="[[mutedSeries]]" x-axis-type="[[xAxisType]]" y-axis-type="[[yAxisType]]" complete-series-config="[[completeSeriesConfig]]" mouse-position="[[mousePosition]]" hover-target="[[mouseRect]]" tooltip-style="light" hard-mute="[[hardMute]]" series-keys="[[_seriesKeys]]">
          </px-vis-tooltip>
          <px-vis-svg-canvas class="inline--flex" id="svg" width="[[width]]" height="[[height]]" margin="[[_internalMargin]]" canvas-context="{{canvasContext}}" canvas-layers="{{canvasLayers}}" canvas-layers-config="[[canvasLayersConfig]]" svg="{{svg}}" px-svg-elem="{{pxSvgElem}}" svg-lower="{{svgLower}}" px-svg-elem-lower="{{pxSvgElemLower}}">
        ...
        <px-vis-clip-path svg="[[layer.1]]" x="[[x]]" y="[[y]]" width="[[width]]" height="[[height]]" margin="[[_internalMargin]]" clip-path="{{clipPath}}" series-clip-path-y-offset="-5" series-clip-path="{{seriesClipPath}}">
        </px-vis-clip-path>
        <!-- scatter series -->
        <template is="dom-repeat" items="[[_seriesKeys]]">
          <template is="dom-if" if="[[_chartTypeScatter(item, completeSeriesConfig)]]" restamp="">
            <template is="dom-if" if="[[renderToCanvas]]" restamp="">
              <px-vis-scatter-canvas canvas-context="[[canvasContext]]" series-id="[[item]]" chart-data="[[_filteredData]]" renderer-type="filteredChartData" complete-series-config="[[completeSeriesConfig]]" width="[[width]]" height="[[height]]" margin="[[_internalMargin]]" x="[[x]]" y="[[_returnYScale(item, completeSeriesConfig, domainChanged)]]" clip-path="" domain-changed="[[domainChanged]]" muted-series="[[mutedSeries]]" serie-to-redraw-on-top="[[serieToRedrawOnTop]]">
              </px-vis-scatter-canvas>
            </template>
            <template is="dom-if" if="[[!renderToCanvas]]" restamp="">
              <px-vis-scatter svg="[[layer.1]]" series-id="[[item]]" chart-data="[[_filteredData]]" x="[[x]]" y="[[_returnYScale(item, completeSeriesConfig, domainChanged)]]" domain-changed="[[domainChanged]]" muted-series="[[mutedSeries]]" clip-path="[[seriesClipPath]]" complete-series-config="[[completeSeriesConfig]]" serie-to-redraw-on-top="[[serieToRedrawOnTop]]">
              </px-vis-scatter>
            </template>
          </template>
          <template is="dom-if" if="[[_chartTypeLine(item, completeSeriesConfig)]]" restamp="">
            <template is="dom-if" if="[[renderToCanvas]]" restamp="">
              <px-vis-line-canvas id="lineCanvas" canvas-context="[[canvasContext]]" renderer-type="filteredChartData" series-id="[[item]]" chart-data="[[_filteredData]]" complete-series-config="[[completeSeriesConfig]]" x="[[x]]" y="[[_returnYScale(item, completeSeriesConfig, domainChanged)]]" domain-changed="[[domainChanged]]" show-gaps="[[showGaps]]" muted-series="[[mutedSeries]]" width="[[width]]" height="[[height]]" margin="[[_internalMargin]]" clip-path="" interpolation-function="[[interpolationFunction]]" serie-to-redraw-on-top="[[serieToRedrawOnTop]]">
              </px-vis-line-canvas>
            </template>
            <template is="dom-if" if="[[!renderToCanvas]]" restamp="">
              <px-vis-line-svg svg="[[layer.1]]" series-id="[[item]]" chart-data="[[_filteredData]]" x="[[x]]" y="[[_returnYScale(item, completeSeriesConfig, domainChanged)]]" show-gaps="[[showGaps]]" domain-changed="[[domainChanged]]" muted-series="[[mutedSeries]]" clip-path="[[seriesClipPath]]" complete-series-config="[[completeSeriesConfig]]" interpolation-function="[[interpolationFunction]]" serie-to-redraw-on-top="[[serieToRedrawOnTop]]">
              </px-vis-line-svg>
            </template>
          </template>
        </template>
        <px-vis-cursor id="cursor" svg="[[layer.2]]" width="[[width]]" height="[[height]]" margin="[[_internalMargin]]" chart-data="[[_filteredData]]" tooltip-data="[[_registerTooltipData]]" complete-series-config="[[completeSeriesConfig]]" clip-path="[[seriesClipPath]]" muted-series="[[mutedSeries]]" hard-mute="[[hardMute]]" series-keys="[[_seriesKeys]]">
        </px-vis-cursor>
      </div>

      <!-- X axis -->
      <px-vis-axis id="xAxis" svg="[[layer.0]]" axis-type="[[xAxisType]]" axis="[[x]]" height="[[height]]" width="[[width]]" margin="[[_internalMargin]]" orientation="[[xAxisLocation]]" complete-series-config="[[completeSeriesConfig]]" prevent-series-bar="" domain-changed="[[domainChanged]]" muted-series="[[mutedSeries]]">
      </px-vis-axis>
      <!-- Y axis -->
      ...
      <px-vis-event svg="[[layer.0]]" event-data="[[eventData]]" x-axis-type="[[xAxisType]]" height="[[height]]" margin="[[_internalMargin]]" x="[[x]]" domain-changed="[[domainChanged]]" event-config="[[eventConfig]]" clip-path="[[clipPath]]" resources="[[resources]]" language="[[language]]" formats="[[formats]]">
      </px-vis-event>
    </div>
